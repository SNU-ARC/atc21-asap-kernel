 ...
 */
static atomic_t swap_in_concur;
int do_swap_page(struct vm_fault *vmf)
{
	...
	if (!page)
		page = lookup_swap_cache(entry, vma_readahead ? vma : NULL,
					 vmf->address);
	if (!page) { // page not exists in swap cache 
		if (vma_readahead)
...
#ifdef ENABLE_ASAP
        if (switch_target_task && is_current_ppthread()) {
                /* clear pte AF if current swapper is ppthreads
                 * * this may occur warning inside of set_pte_at() in   ARM64 */
                pte = pte_mkold(pte);
        }
#endif
		...
		unsigned int flags)
{
	struct vm_fault vmf = {
		.vma = vma,
		.address = address & PAGE_MASK, // now address is page-aligned
		.flags = flags,
