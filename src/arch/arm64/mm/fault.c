...
#ifdef ENABLE_ASAP
DEFINE_SPINLOCK(anon_fault_buf_lock);
#endif
	...
	if (fault & VM_FAULT_RETRY) {
		...
		if (atomic_read(&global_switch_flag)) {
			if ( (fault & VM_FAULT_SWAP) &&    
                                !anon_ptt_is_in(current_anon_ptt, mm->owner->tgid, addr & PAGE_MASK)) { 
                                spin_lock_irqsave(&anon_fault_buf_lock, flags);
				anon_fault_buf[anon_fault_buf_len].tgid = mm->owner->tgid;
				anon_fault_buf[anon_fault_buf_len].va = addr & PAGE_MASK;
				anon_fault_buf_len = (anon_fault_buf_len + 1)%FAULT_BUF_SZ;
                                spin_unlock_irqrestore(&anon_fault_buf_lock, flags);
                                if (anon_ptt_is_in(current_anon_ptt, mm->owner->tgid, addr & PAGE_MASK)) {
                                        atomic_inc(&anon_app_hit_cnt);
                                }
                        }
		}
	}
